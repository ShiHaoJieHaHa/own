...
// This is the ols search Filter method used in navigator.
	...
	filterGrid : function(gridid,p){
		p = $.extend({
			...
			autosearch: true, // if set to false a serch menu should be enabled.
		...
		return this.each(function(){
			...
			var formFill = function(){
				...
				if(self.p.enableSearch === true){
					sb = "<input type='menu' id='sButton' class='"+self.p.buttonclass+"' value='"+self.p.searchButton+"'/>";
					$(td).append(sb);
				...
				if(self.p.enableClear === true) {
					cb = "<input type='menu' id='cButton' class='"+self.p.buttonclass+"' value='"+self.p.clearButton+"'/>";
					$(td).append(cb);
