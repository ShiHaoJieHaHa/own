...
TableTools = function( oDT, oOpts )
{
	...
	this.s = {
		...
		/**
		 * A unique ID is assigned to each menu in each instance
...
TableTools.prototype = {
	 ...
	 * included for a menu. This is particularly useful for PDF creation, where we can use the
	...
	/**
	 * Pass text to a flash menu instance, which will be used on the menu's click handler
	 *  @param   {Object} clip Flash menu object
	...
	"_fnButtonDefinations": function ( buttonSet, wrapper )
	{
		...
		for ( var i=0, iLen=buttonSet.length ; i<iLen ; i++ )
		{
			if ( typeof buttonSet[i] == "string" )
			{
				...
				{
					alert( "TableTools: Warning - unknown menu type: "+buttonSet[i] );
					continue;
				}
			...
			else
			{
				...
				{
					alert( "TableTools: Warning - unknown menu type: "+buttonSet[i].sExtends );
					continue;
				}
	...
	/**
	 * Create and configure a TableTools menu
		...
		if ( this.s.dt.iTabIndex !== -1 ) {
			$(nButton)
				...
				.on( 'mousedown.DTTT', function (e) {
					...
					// menu, focus is used only for the keyboard navigation.
					// But using preventDefault for the flash buttons stops the
					// flash action. However, it is not the menu that gets
	...
	/**
	 * Create the DOM needed for the menu and apply some base properties. All buttons start here
	 *  @method  _fnButtonBase
	 *  @param   {o} oConfig Button configuration object
	 *  @returns {Node} DIV element for the menu
	 *  @private
	 */
	...
	/**
	 * Create a collection menu, when activated will present a drop down list of other buttons
	...
	/**
	 * Configure a text based menu for interaction events
	...
	/**
	 * Configure a flash based menu for interaction events
...
/**
 * @namespace Default menu configurations
 */
TableTools.BUTTONS = {
 ...
 *     1. node - menu element
 *     2. object - configuration object for this menu
 *     3. object - ZeroClipboard reference (flash menu only)
 *     4. string - Returned string from Flash (flash menu only - and only on 'complete')
 */
...
TableTools.classes_themeroller = {
	...
	"buttons": {
		"normal": "DTTT_button ui-menu ui-state-default"
	},
	"collection": {
...
TableTools.DEFAULTS = {
	...
	"oTags": {
		...
		"button": "a", // We really want to use buttons here, but Firefox and IE ignore the
		                 // click on the Flash element in the menu (but not mouse[in|out]).
		"liner": "span",
		"collection": {
			"container": "div",
			"button": "a",
